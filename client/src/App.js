...
export default function App() {
  ...
  async function playAiTtsOnce(nextLesson) {
    const text = nextLesson?.segments
      ?.slice(0, 3)
      .map((s) => s.text)
      .join("\n")
      .trim();
    ...
    <div style={styles.page}>
      ...
      <div style={styles.main}>
        <section style={styles.blackboard}>
          <h2 style={styles.boardTitle}>{lesson?.title || "黑板"}</h2>
          ...
          {lesson?.blackboard?.length ? (
            ...
            <div style={styles.emptyBoard}>
              输入语法点后，AI 会把核心感觉、中文意思、接续、使用场景和例句写在这里。
            </div>
          )}
        </section>
      ...
      <div style={styles.footer}>
        后端：{API_BASE} / Railway 修改后记得点 Deploy
      </div>
    </div>
  );
}
